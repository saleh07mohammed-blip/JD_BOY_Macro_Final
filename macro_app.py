...
# ⚙️ إعدادات التحديث
# ------------------------------------------------------------
APP_VERSION = "1.0.4"  # 🚨 الإصدار الحالي (الجديد)
...
class App:
    def __init__(self, root):
        ...
        # 🚨 تغيير عنوان البرنامج لتمييز الإصدار 1.0.4 
    ...
    def _run_update_check(self):
        ...
        try:
            ...
            if latest_v > app_v:
                ...
                # 💡 المنطق الجديد: إذا كان الإصدار الحالي أقل من 1.0.3، أو إذا كان هناك تحديث جديد متوفر
                version_to_force = parse_version("1.0.3")

                # هل النسخة الحالية (مثل v1.0.4) وجدت إصدار أحدث (مثل v1.0.5)؟
                is_mandatory_update = latest_v > app_v
                
                # هل النسخة الحالية هي نسخة قديمة (مثل v1.0.1) ويجب إجبارها على التحديث؟
                # (هذا المنطق سيعمل فقط إذا كان الكود في هذه النسخة القديمة نفسها!)
                is_too_old_to_run = app_v < version_to_force

                if is_mandatory_update or is_too_old_to_run:
                ...
                # هذا الجزء لن يتم تنفيذه عمليا ما دمنا نستخدم المنطق الإجباري
                else:
                    self.root.after(0, lambda: self._log("🎉 **تم العثور على تحديث!** هل تريد التحديث الآن؟"))
                    if messagebox.askyesno("تحديث اختياري", f"تم العثور على الإصدار {latest_version_str}. هل تريد التحميل الآن؟"): 
                        self.root.after(0, lambda: self.perform_update(download_url))
                ...
                # (في حال كان الملف JSON يشير بالخطأ إلى إصدار أقدم من إصدارنا)
                self.root.after(0, lambda: self._log("✅ برنامجك هو أحدث إصدار."))
